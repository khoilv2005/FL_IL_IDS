...
# =============================================================================
# TEST: NICE (Neurogenesis Inspired Contextual Encoding)
# =============================================================================
...
class TestNICE:
    ...
    def test_nice_trainer_init(self):
        ...
        assert trainer.num_neurons_per_layer == 100
        assert trainer.memory_per_episode == 500
        assert trainer.current_task == 0

    def test_nice_trainer_custom_params(self):
        """NICETrainer should accept custom params."""
        trainer = NICETrainer(tau=0.9, num_neurons_per_layer=50, memory_per_episode=200)
        assert trainer.tau == 0.9
        assert trainer.num_neurons_per_layer == 50
        assert trainer.memory_per_episode == 200
    ...
    def test_nice_aggregator_creation(self):
        ...
        from fed_learning.strategies.incremental.nice import NICEAggregator
    ...
    def test_nice_aggregator_fedavg(self):
        ...
        # FedAvg should give simple average
        assert torch.allclose(agg["w"], torch.tensor([1.0]), atol=1e-5)
    ...
    def test_nice_model_forward(self):
        ...
        model = NICEModel(input_shape=(100, 1), num_classes=34)
        x = torch.randn(4, 100)  # batch=4, seq_len=100

        output = model(x)
        assert output.shape == (4, 34)

    def test_nice_model_neuron_ages(self):
        """NICEModel should track neuron ages."""
        from fed_learning.models.nice_model import NICEModel

        model = NICEModel(input_shape=(100, 1), num_classes=34)

        # Check initial ages are 0
        assert "conv1" in model._neuron_ages
        assert "conv2" in model._neuron_ages
        assert "fc2" in model._neuron_ages
        assert torch.all(model._neuron_ages["conv1"] == 0)
    ...
    def test_nice_client_no_buffer(self):
        ...
        # Create client with dummy data
        X = torch.randn(100, 50)
        y = torch.randint(0, 10, (100,))
        client = NICEClient(0, X, y)

        # Should have buffer stats showing no buffer
