...
class FedAvgEWCTrainer(EWCMixin, FedAvgTrainer):
    ...
    Class này kết hợp:
    - cơ chế train cơ bản của FedAvg
    - penalty chống quên của EWC

    Aggregation phía server vẫn là FedAvg như cũ; điểm khác biệt nằm ở local
    loss tại mỗi client.
    """

    def __init__(self, **kwargs):
        """
        Khởi tạo trainer kết hợp FedAvg và EWC.

        Hàm này tách các tham số EWC ra khỏi kwargs, khởi tạo EWCMixin trước,
        sau đó khởi tạo FedAvgTrainer. Thứ tự này đảm bảo object vừa có trạng
        thái EWC, vừa có hành vi trainer của FedAvg.
        """
        ewc_args = {
...
class FedProxEWCTrainer(EWCMixin, FedProxTrainer):
    ...
    Class này kết hợp 2 regularization cùng lúc:
    - proximal term của FedProx để giữ client gần global model
    - Fisher penalty của EWC để giữ trí nhớ cũ cho tham số quan trọng
    """

    def __init__(self, **kwargs):
        """
        Khởi tạo trainer kết hợp FedProx và EWC.

        Tương tự FedAvgEWCTrainer, nhưng ngoài tham số EWC còn tách thêm `mu`
        của FedProx. Kết quả là loss nền sẽ là CE + proximal term, sau đó mới
        được EWCMixin cộng thêm penalty EWC.
        """
        ewc_args = {
            "ewc_lambda": kwargs.pop("ewc_lambda", 1000.0),
            "fisher_samples": kwargs.pop("fisher_samples", 200),
