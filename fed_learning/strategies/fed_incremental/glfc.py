...
def get_one_hot(target: torch.Tensor, num_class: int, device: str) -> torch.Tensor:
    """Chuyển nhãn dạng chỉ số sang one-hot encoding."""
...
def compute_entropy(probs: torch.Tensor) -> torch.Tensor:
    """Tính entropy của phân phối xác suất cho từng mẫu."""
...
class GLFCTrainer(BaseTrainer):
    ...
    Ý tưởng chính:
    - Bù quên cục bộ bằng distillation với old model.
    - Dùng class-aware gradient compensation để cân bằng gradient.
    - Dùng entropy signal để biết khi nào cần cập nhật exemplar/proxy.
    ...
    def set_task(self, task_id: int, new_classes: List[int]):
        """Đồng bộ class cũ/mới và reset cache khi bắt đầu task mới."""
    ...
    def save_model_snapshot(self, model: nn.Module):
        """Lưu snapshot model hiện tại để dùng cho distillation ở task sau."""
    ...
    def update_proxy_server_models(self, model: nn.Module, perf: float):
        """
        Cập nhật các best model mà proxy server đang theo dõi.

        Proxy server giữ lại 2 mốc model tốt nhất để client dùng khi cần
        bù quên toàn cục ở các task sau.
        """
        state_dict = OrderedDict(
            (k, v.cpu().clone()) for k, v in model.state_dict().items()
        )
    ...
    def get_old_models(self):
        """
        Lấy các old model state cho bước distillation của GLFC.

        Thường client sẽ chọn giữa `best_model_1` và `best_model_2`
        tùy vào entropy signal.
        """
    ...
    def load_old_model(
        ...
        """
        Load old model để làm teacher cho knowledge distillation.

        Thứ tự ưu tiên:
        - nếu có entropy signal thì ưu tiên model tốt mới hơn
        - nếu không thì dùng model tốt cũ hơn
        - nếu proxy chưa có thì fallback về snapshot gần nhất
        """
    ...
    def compute_entropy_signal(
        ...
        """
        Phát hiện entropy signal để kích hoạt cơ chế bù quên toàn cục.

        Nếu entropy trung bình tăng vượt ngưỡng, thuật toán coi đó là dấu hiệu
        kiến thức mới đang làm lệch phân phối dự đoán và cần cập nhật bộ nhớ.
        """
        model.eval()
        all_entropy = []
    ...
    def efficient_old_class_weight(
        ...
        """
        Tính trọng số class-aware gradient compensation cho từng mẫu.

        Mẫu nào khó hơn hoặc lệch dự đoán nhiều hơn sẽ được đẩy trọng số cao hơn,
        đồng thời nhóm lớp cũ và lớp mới được chuẩn hóa riêng để tránh mất cân bằng.
        """
    ...
    def compute_loss(
        ...
        **kwargs,
        ...
        """
        Tính GLFC loss với cơ chế bù quên cục bộ.

        - Nếu chưa có old model: chỉ dùng loss hiện tại có trọng số.
        - Nếu đã có old model: trộn `loss_cur` và `loss_old` để giữ kiến thức cũ.
        """
    ...
    def update_forgetting(self, task_accuracies: Dict[int, float]):
        """Cập nhật accuracy tracking để tính Average Forgetting."""
    ...
    def cleanup(self):
        """Dọn cache và thư mục tạm của GLFC."""
        self._cached_old_model = None
...
class GLFCAggregator(BaseAggregator):
    ...
    GLFC không thay đổi công thức aggregate phía server.
    Phần đặc thù nằm ở trainer và cơ chế proxy server.
    """
