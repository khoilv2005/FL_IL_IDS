...
class FedCBDRServer:
    ...
    def coordinate_gdr(self, participating_clients=None, verbose: bool = True):
        ...
        for idx, client in enumerate(clients):
            ...
            if verbose:
                print(
                    f"   Processing client {client.client_id} ({idx + 1}/{len(clients)})..."
                )
            ...
            features = client.extract_features_for_gdr(
                self.global_model, batch_size=gdr_batch
            )
            ...
            n_select = min(buffer_per_class * len(client.current_classes), len(scores))
            ...
            client.update_replay_buffer(
                self.global_model, selected_indices=top_indices, use_herding=use_herding
            )
