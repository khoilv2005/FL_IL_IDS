"""
PlexusNICE Worker - Multi-GPU training worker for PlexusNICE clients.

Combines:
- PlexusWorker pattern (sample/aggregator selection handled by server)
- NICEWorker pattern (NICEModel, neuron ages/masks, phase-based training)
...
class PlexusNICEWorker(BaseGPUWorker):
    """Worker for PlexusNICE algorithm with NICEModel and neuron age/mask transfer."""
    ...
    def run(self):
        """Override run for PlexusNICE-specific logging."""
        import time
        gpu_start = time.time()
        ...
        print(f"      [{self.device_name}] PlexusNICE: "
...
def train_plexus_nice_clients_on_gpu(
    gpu_id: int,
    clients: list,
    global_params: OrderedDict,
    config: Dict,
    ...
    """
    Train PlexusNICE clients on a specific GPU.

    Args:
        gpu_id: GPU ID (0, 1, 2, ...)
        clients: List of PlexusNICEClient instances
        ...
        trainer: PlexusNICETrainer instance
        use_cpu: Whether to use CPU instead of GPU
    """
    worker = PlexusNICEWorker(
